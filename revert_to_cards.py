...
pattern_member = re.compile(
    r'<div class="polaroid-card" data-member data-role="(.*?)" data-rank="(.*?)".*?<img alt="(.*?) avatar" src="(.*?)" />.*?<h3 class="polaroid-card__name">(.*?)</h3>.*?<p class="polaroid-card__role">(.*?)</p>',
    re.DOTALL
)
...
pattern_grid = r'<div class="scrapbook-grid">.*?      </div>'
new_content = re.sub(pattern_grid, new_grid, content, flags=re.DOTALL)

# Add cache buster
new_content = new_content.replace('href="./styles.css"', 'href="./styles.css?v=3"')
new_content = new_content.replace('href="./styles.css?v=2"', 'href="./styles.css?v=3"')

with open(html_file, 'w', encoding='utf-8') as f:
    f.write(new_content)

print(f'Updated index.html back to roster__grid with {len(members)} cards.')
